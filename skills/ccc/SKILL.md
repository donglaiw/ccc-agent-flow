...
4. For a new `run`, confirm `codex exec --help` and `codex exec review --help` work, or initialize the run as blocked.
...
2. Run codex exec or codex exec review with --output-last-message state/<review-stage>.codex.raw.md.
...
If the Codex CLI exits non-zero, produces no raw transcript, or the output does not clearly support a verdict, stop with `Status: blocked`.
