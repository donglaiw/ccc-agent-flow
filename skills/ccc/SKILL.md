...
7. For reviewer stages, print the exact `/codex:... --wait` slash command for the user to run in the same Claude Code session, then continue from the plugin output.
...
If the plugin output is ambiguous, unsupported by the raw transcript, or lacks a clear verdict, stop with `Status: blocked`.
